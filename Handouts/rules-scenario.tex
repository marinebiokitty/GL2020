...
\textcolor{red}{\subsection{Curses:}}
\textcolor{red}{Some characters have the ability to create curses. These are represented by item cards that describe their effects. Some curses will have associated abilities or greensheets. Curses begin as latent magical effects, but can be activated in one of several ways. Once activated, write ``expended'' on the card and the person upon whom the curse is active should carry it around with them until the effect ends to remind themselves of the effect. ``Expended'' curses no longer represent physical items, they are just OOC reminders.}

\textcolor{red}{\subsubsection{Activating a Curse:}}
\textcolor{red}{Unless you know otherwise (e.g. The item card for the curse says something different), you may:}
\begin{itemize}
	\textcolor{red}
	\item You may always chose to activate a curse on yourself. (write ``expended'' on the card)
	\item You may activate a curse on a willing target. Hand them the item card.
	\item You may activate a curse on an unconscious or restrained target. Hand them the item card.
	\item You may use a waylay with a 5-count to secretly activate a curse on someone.
	\item Some mechanics may specify other ways to activate a specific curse on someone.
\end{itemize}

\textcolor{red}{Some curses may specify more stringent requirements for activation. Such items will say so.}
