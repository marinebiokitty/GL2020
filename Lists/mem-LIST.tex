...
\NEW{MemFold}{\mCPacket}{
  \s\MYname	{C-Packet: Open only if prompted to.}
...
\NEW{MemEnvelope}{\mPacketThree}{
  \s\MYname	{Packet 3: Open when prompted to.}
...
\NEW{MemFold}{\mWOne}{
  ...
  \s\MYtext	{Some scholars have observed an almost angry or rueful tone in Kero's teachings around respecting intellectual property, an almost sorrowful tone in Luminary's teachings around refraining from violence, and an almost defiant tone in Ebb's and Flow's teachings around Balance and repaying others in kind. When it comes to the universal prohibition against murder, a few scholars have noted a hint of regret in the teachings of the Patron Gods. While many theories abound, none really resonate with the \textbf{full truth} yet.}
}
...
\NEW{MemFold}{\mGreenHeadband}{
  \s\MYname	{Open if you see a BLUE headband.}
...
\NEW{MemFold}{\mSisterMemory}{
  \s\MYname	{Open if you complete the ritual to look into the past and learn what happened to your sister (read ``Sign J - 2 Day Dreaming of the Past’’ first).}
