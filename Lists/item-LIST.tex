...
%%COS Character Items
\NEW{ItemEnvelope}{\iBeans}{ %%Amy or Ghost starts with it
  \s\MYname	{Magic Beans}
  \s\MYnumber	{0000}
  \s\MYtext	{Magic Beans?}
  \s\MYcontents	{Status: Not ready to plant}
}
...
\NEW{Item}{\iBeans}{ %%Prototype beans. Does Amy or GHost start with these?
  \s\MYname	{Nondescript Beans}
  \s\MYnumber	{0000}
  \s\MYtext	{A handful of beans. Were they\ldots buzzing for a moment?}
}

\NEW{Item}{\iMagicBeans}{ %%Magic beans. Need to store these somewhere for them to pick up at the end of completing their development
  \s\MYname	{Magic Beans}
  \s\MYnumber	{0000}
  \s\MYtext	{A handful of beans. They buzz loudly and even rattle a little in your hand. \textbf{Tell a GM immediately if you plant them somewhere!}}
}
